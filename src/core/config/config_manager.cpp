...
Result<Config> ConfigManager::parse_json(const nlohmann::json& j) {
    ...
    // Parse FX rates (optional, overrides currency.hpp defaults)
    if (j.contains("fx_rates") && j["fx_rates"].is_object()) {
        for (auto& [key, val] : j["fx_rates"].items()) {
            if (val.is_number()) {
                config.fx_rates[key] = val.get<double>();
            }
        }
    }
...
Result<AccountConfig> ConfigManager::parse_account(const nlohmann::json& j) {
    ...
    // IBKR account ID (optional, links config name to IBKR account number)
    if (j.contains("ibkr_account_id")) {
        account.ibkr_account_id = j["ibkr_account_id"].get<std::string>();
    }

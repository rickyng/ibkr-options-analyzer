...
utils::Result<void> CSVExporter::export_strategies_csv(
    ...
    try {
        ...
        for (size_t i = 0; i < strategies.size(); ++i) {
            ...
            file << escape_csv_field(analysis::strategy_type_to_string(strategy.type)) << ","
...
utils::Result<void> CSVExporter::export_summary_csv(
    ...
    try {
        ...
        file << "Total Strategies," << strategies.size() << "\n";
        file << "Total Max Profit," << std::fixed << std::setprecision(2)
             << total_max_profit << "\n";
        file << "Total Max Loss," << total_max_loss << "\n";
        file << "Current Loss," << total_loss_current << "\n";
        file << "5% Loss," << total_loss_5pct << "\n";
        file << "Expiring in 7 Days," << positions_expiring_soon << "\n";

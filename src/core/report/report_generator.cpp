...
std::string ReportGenerator::generate_portfolio_summary(
    ...
    // Calculate portfolio metrics inline
    size_t total_positions = strategies.size();
    ...
    for (const auto& m : metrics) {
        total_max_profit += m.max_profit;
        if (std::isfinite(m.max_loss)) total_max_loss += m.max_loss;
        total_loss_current += m.max_loss_current;
        total_loss_5pct += m.max_loss_5pct;
        if (m.days_to_expiry < 7 && m.days_to_expiry >= 0) positions_expiring_soon++;
    }

    section << std::left << std::setw(30) << "Total Positions:"
            << std::right << std::setw(20) << total_positions << "\n";

    section << std::left << std::setw(30) << "Total Strategies:"
            << std::right << std::setw(20) << total_positions << "\n";

    section << std::left << std::setw(30) << "Total Max Profit:"
            << std::right << std::setw(20) << format_currency(total_max_profit) << "\n";

    section << std::left << std::setw(30) << "Total Max Loss:"
            << std::right << std::setw(20) << format_currency(total_max_loss) << "\n";

    section << std::left << std::setw(30) << "Current Loss:"
            << std::right << std::setw(20) << format_currency(total_loss_current) << "\n";

    section << std::left << std::setw(30) << "5% Loss:"
            << std::right << std::setw(20) << format_currency(total_loss_5pct) << "\n";

    section << std::left << std::setw(30) << "Expiring in 7 Days:"
            << std::right << std::setw(15) << positions_expiring_soon
            << " positions\n";
        ...
        for (const auto& [underlying, strats] : by_underlying) {
            ...
            for (const auto& [strategy, metric] : strats) {
                ...
                if (strategy->legs.size() == 1) {
                    ...
                    section << "    " << analysis::strategy_type_to_string(strategy->type) << "\n";
...
std::string ReportGenerator::generate_strategies_by_type(
    ...
    for (const auto& [type, strats] : by_type) {
        section << analysis::strategy_type_to_string(type)
                << " (" << strats.size() << " strateg";
        if (strats.size() == 1) {
            section << "y";
        } else {
    ...
    for (const auto& [strategy, metric] : this_week) {
        if (strategy->legs.size() == 1) {
            ...
            section << "  " << strategy->expiry << "  " << std::setw(6) << std::left
                    << strategy->underlying << " "
                    << analysis::strategy_type_to_string(strategy->type)
                    << "  (" << metric->days_to_expiry << " days)\n";
        }
    }
    section << "\n";
...
std::string ReportGenerator::generate_risk_analysis(
    ...
    std::sort(by_max_loss.begin(), by_max_loss.end(),
        [](const auto& a, const auto& b) {
            const bool a_fin = std::isfinite(a.second->max_loss);
            const bool b_fin = std::isfinite(b.second->max_loss);
            if (!a_fin && !b_fin) return false; // both non-finite: equal
            if (!a_fin) return false;            // non-finite sorts to end
            if (!b_fin) return true;             // finite sorts before non-finite

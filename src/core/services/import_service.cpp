...
Result<ImportStats> ImportService::import_files(
    ...
    for (const auto& file : files) {
        ...
        auto account_result = database_.accounts().get_or_create_account(account_name);
        if (!account_result) {
            Logger::error("Failed to get account: {}", account_result.error().message);
            continue;
        }
        ...
        if (clear_existing && !parse_data.open_positions.empty()) {
            auto clear_result = database_.trades().clear_open_positions(account_id);
            if (!clear_result) {
        ...
        if (!parse_data.trades.empty()) {
            auto import_result = database_.trades().import_trades(account_id, parse_data.trades);
            if (!import_result) {
        ...
        if (!parse_data.stock_trades.empty()) {
            auto import_result = database_.trades().import_stock_trades(account_id, parse_data.stock_trades);
            if (!import_result) {
                Logger::error("Failed to import stock trades: {}", import_result.error().format());
            } else {
                stats.stock_trades_imported += parse_data.stock_trades.size();
                Logger::info("Imported {} stock trades", parse_data.stock_trades.size());
            }
        }

        if (!parse_data.open_positions.empty()) {
            auto import_result = database_.trades().import_open_positions(account_id, parse_data.open_positions);
            if (!import_result) {
    ...
    auto trades_count = database_.trades().get_trades_count();
    auto positions_count = database_.trades().get_open_positions_count();

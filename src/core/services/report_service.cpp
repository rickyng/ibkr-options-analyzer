...
Result<ReportData> ReportService::gather_report_data(
    ...
    auto pos_result = database_.positions().get_all_positions(account_filter);
    if (!pos_result) {
    ...
    // Load risk summaries
    auto risk_result = database_.positions().get_consolidated_risk();
    if (!risk_result) {
    ...
    // Load exposures
    auto exp_result = database_.positions().get_underlying_exposure();
    if (!exp_result) {
        return Error{"Failed to load exposures", exp_result.error().message};
    }
    data.exposures = *exp_result;

    // Wrap each position as a single-leg strategy for reporting
    for (const auto& pos : data.positions) {
        analysis::Strategy strategy;
        strategy.type = (pos.quantity < 0)
            ? (pos.right == 'P' ? analysis::Strategy::Type::NakedShortPut
                                : analysis::Strategy::Type::NakedShortCall)
            : analysis::Strategy::Type::Unknown;
        strategy.underlying = pos.underlying;
        strategy.expiry = pos.expiry;
        strategy.currency = pos.currency;
        strategy.legs.push_back(pos);

        if (!underlying_filter.empty() && strategy.underlying != underlying_filter) {
            continue;
        }

        data.metrics.push_back(analysis::RiskCalculator::calculate_risk(strategy));
        data.strategies.push_back(std::move(strategy));
    }

...
PortfolioView PortfolioService::build_portfolio_view(
    ...
    for (auto& pos : positions) {
        ...
        // Per currency policy: no conversion at read time

        int dte = analysis::RiskCalculator::calculate_days_to_expiry(pos.expiry);

        // P&L calculation (for short positions) — values already in USD
        if (pos.quantity < 0) {
            double entry_total = std::abs(pos.quantity) * pp.position.entry_premium
                               * pos.multiplier;
            view.total_premium_collected += entry_total;

            pp.current_premium = pp.position.mark_price > 0 ? pp.position.mark_price : pp.position.entry_premium;
        ...
        if (pp.has_price && pos.right == 'P' && pos.quantity < 0) {
            ...
            pp.otm_percent = ((cp - pp.position.strike) / cp) * 100.0;

            if (cp < pp.position.strike) {
                pp.risk_alert = "ITM";
                view.itm_count++;
            ...
            if (dte > 0 && pp.position.strike > 0) {
                pp.annualized_yield = (pp.position.entry_premium / pp.position.strike) * (365.0 / dte) * 100.0;
            }
        }
        ...
        // Current loss and 5% loss per account — values already in USD
        if (pos.quantity < 0) {
            // Current loss: using actual market price
            if (pp.has_price) {
                double intrinsic = (pos.right == 'P')
                    ? std::max(0.0, pp.position.strike - pp.current_price)
                    : std::max(0.0, pp.current_price - pp.position.strike);
                double loss_cur = std::max(0.0, intrinsic - pp.position.entry_premium)
                               * pos.multiplier * std::abs(pos.quantity);
                view.loss_current[pp.account_name] += loss_cur;
            }
            // 5% loss: market drops 5% below current price (short puts only)
            if (pp.has_price && pos.right == 'P') {
                double scenario_price = pp.current_price * 0.95;
                double intrinsic_5 = std::max(0.0, pp.position.strike - scenario_price);
                double loss_5 = std::max(0.0, intrinsic_5 - pp.position.entry_premium)
                             * pos.multiplier * std::abs(pos.quantity);
                view.loss_5pct[pp.account_name] += loss_5;
            }
        }

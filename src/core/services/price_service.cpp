...
utils::Result<int> PriceService::fetch_and_cache_earnings(
    const std::vector<std::string>& symbols) {

    auto earnings = fetcher_.fetch_earnings_dates(symbols);
    int count = static_cast<int>(earnings.size());
    auto save_result = cache_service_->save_all_earnings_dates(earnings);

...
// --- Price caching ---
//
// CACHE SCOPE: All cached prices are keyed by trading_date (the current
// trading day). Prices fetched at market open remain valid all day.
// Intraday price movements are NOT reflected until the cache is explicitly
// refreshed via the `refresh` CLI command.
// =============================================================================
...
Result<utils::StockPrice> PriceCacheService::get_price(const std::string& symbol) {
    ...
    // Convert to USD before caching so all DB values are in a single currency.
    // The dashboard reads cached_prices directly and must not do conversion.
    std::string currency = utils::deduce_currency(symbol);
    result->price = utils::CurrencyConverter().convert(result->price, currency);
...
// NOTE: Option chain strike prices are stored in their native currency
// (matching Yahoo Finance's convention). Strikes are relative to the
// underlying price in the same currency, which is consistent with how
...
Result<void> PriceCacheService::save_earnings_date(
    const std::string& symbol,
    const std::string& earnings_date,
    ...
    try {
        SQLite::Statement upsert(*sqlite_db,
            "INSERT OR REPLACE INTO cached_earnings_dates "
            "(symbol, next_earnings_date, source, fetched_at, trading_date) "
            "VALUES (?, ?, ?, datetime('now'), ?)");
        upsert.bind(1, symbol);
        upsert.bind(2, earnings_date);
        upsert.bind(3, source);
        upsert.bind(4, trading_date);
        upsert.exec();
        return Result<void>{};
    } catch (const std::exception& e) {
        return Error{"Failed to save earnings date", std::string(e.what())};
    }
}

Result<void> PriceCacheService::save_all_earnings_dates(
    const std::map<std::string, utils::EarningsDate>& earnings) {
    std::string trading_date = get_current_trading_date();
    int saved = 0;
    for (const auto& [symbol, ed] : earnings) {
        auto result = save_earnings_date(symbol, ed.next_earnings_date, ed.source, trading_date);
        if (result) {
            ++saved;
        } else {
            Logger::warn("Failed to cache earnings for {}: {}", symbol, result.error().message);
        }
    }
    Logger::info("Cached {} earnings dates", saved);
    return Result<void>{};
}

            ...
            for (size_t i = 0; i < row_count; ++i) {
                ...
                if (has_trade_data) {
                    if (is_option) {
                        ...
                        // Only import actual stock trades — skip CUR, CASH, FX, etc.
                        auto ac = row_map.find("AssetClass");
                        if (ac != row_map.end() && ac->second == "STK") {
                            auto stock_result = parse_stock_trade_row(row_map);
                            if (stock_result) {
                                result.stock_trades.push_back(*stock_result);
                                result.stock_rows++;
                            }
                        } else {
                            result.skipped_rows++;
...
std::optional<StockTradeRecord> CSVParser::parse_stock_trade_row(
    const std::map<std::string, std::string>& row) {

    StockTradeRecord stock;
    stock.symbol = get_column(row, "Symbol");
    if (stock.symbol.empty()) return std::nullopt;

    stock.description = get_column(row, "Description");
    stock.trade_date = get_column(row, "TradeDate");
    stock.quantity = parse_double(get_column(row, "Quantity"));
    stock.trade_price = parse_double(get_column(row, "TradePrice"));
    stock.proceeds = parse_double(get_column(row, "Proceeds"));
    stock.commission = parse_double(get_column(row, "IBCommission"));
    stock.net_cash = parse_double(get_column(row, "NetCash"));
    stock.notes_codes = get_column(row, "Notes/Codes");

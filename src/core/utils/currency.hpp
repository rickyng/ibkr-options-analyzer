...
public:
    CurrencyConverter() : rates_(default_rates()) {}
    explicit CurrencyConverter(const std::string& base_currency)
        : base_currency_(base_currency.empty() ? "USD" : base_currency)
        , rates_(default_rates()) {}

    /// Construct with custom rates that override the hardcoded defaults.
    explicit CurrencyConverter(const std::map<std::string, double>& custom_rates)
        : rates_(default_rates()) {
        for (const auto& [k, v] : custom_rates) {
            rates_[k] = v;
        }
    }
...
private:
    ...
    static std::map<std::string, double> default_rates() {
        return {
            {"USD", 1.0},
            {"HKD", 0.13},
            {"JPY", 0.0067},

...
#include "db/position_repository.hpp"
#include "db/account_repository.hpp"
...
public:
    ...
    // --- Portfolio command JSON ---

    static nlohmann::json portfolio(const services::PortfolioView& view);
     ...
     */
    static nlohmann::json account(const db::AccountRepository::AccountInfo& info);

    /**
     * Serialize list of accounts.
     */
    static nlohmann::json accounts(const std::vector<db::AccountRepository::AccountInfo>& accounts);
    ...
    /**
     * Serialize refresh command result.
     */
    static nlohmann::json refresh_result(

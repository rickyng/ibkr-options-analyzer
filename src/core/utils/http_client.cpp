...
std::string sanitize_url(const std::string& url) {
    ...
    auto pos = result.find("apikey=");
    if (pos != std::string::npos) {
        auto start = pos + 7; // len("apikey=")
        auto end = result.find('&', start);
        result.replace(start, end == std::string::npos ? result.size() - start : end - start, "REDACTED");
    }
    return result;
}
} // namespace
...
Result<HttpResponse> HttpClient::get(const std::string& path,
    ...
    Logger::debug("HTTP GET: {}{}", base_url_, sanitize_url(full_path));

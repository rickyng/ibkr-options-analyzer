...
static std::string map_symbol_to_yahoo(const std::string& symbol) {
    ...
    // Numeric-only symbols: default to .HK suffix (4-digit zero-padded)
    ...
    if (is_numeric && symbol.length() <= 5) {
        // Yahoo Finance requires HK tickers to be 4-digit zero-padded (e.g., 700 → 0700.HK)
...
PriceFetcher::PriceFetcher() {
    ...
    // Yahoo Finance quote page (for earnings scraping)
    yahoo_quote_client_ = std::make_unique<HttpClient>(
        "https://finance.yahoo.com",
        "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7)",
        10,
        1,   // 1 retry (scraping is fragile, don't over-retry)
        2000
    );
    yahoo_quote_client_->set_header("Accept", "text/html");
    yahoo_quote_client_->set_header("Accept-Encoding", "gzip");
}
...
Result<OptionChainData> PriceFetcher::fetch_option_chain(const std::string& symbol) {
    ...
    // Fallback to synthetic chain if Alpha Vantage key is set
    if (!alpha_vantage_key_.empty()) {
...
Result<OptionChainData> PriceFetcher::fetch_option_chain_from_yahoo(const std::string& symbol) {
    ...
    if (response && response->status_code == 200) {
        try {
            ...
            Logger::debug("JSON parse error in option chain base fetch: {}", e.what());
        }
    }
    ...
    for (const auto& exp : chain.expirations) {
        ...
        try {
            ...
            Logger::debug("JSON parse error in option chain per-expiry fetch: {}", e.what());
            continue;
        }
    }
...
// --- Earnings date scraping from Yahoo Finance quote page ---

// Parse "Mon DD, YYYY" (e.g. "Jul 30, 2026") → "2026-07-30"
static std::string _parse_month_day_year(const std::string& input) {
    ...
    // Parse: "Jul 30, 2026" or "July 30, 2026"
    std::istringstream iss(input);
    std::string month_str;
    int day{0}, year{0};
    char comma;

    iss >> month_str >> day >> comma >> year;
    if (iss.fail()) return "";

    // Match 3-char abbreviation or full name
    std::string key = month_str.substr(0, 3);
    // Capitalize first letter
    if (!key.empty()) key[0] = static_cast<char>(std::toupper(key[0]));

    auto it = months.find(key);
    if (it == months.end()) return "";

    std::ostringstream oss;
    oss << year << "-"
        << std::setfill('0') << std::setw(2) << it->second << "-"
        << std::setfill('0') << std::setw(2) << day;
    return oss.str();
}

Result<EarningsDate> PriceFetcher::fetch_earnings_from_yahoo_quote(const std::string& symbol) {
    std::string yahoo_symbol = map_symbol_to_yahoo(symbol);
    if (yahoo_symbol.empty()) {
        return Error{"Invalid symbol for earnings fetch", symbol};
    }

    Logger::debug("Scraping earnings for {} (yahoo: {})", symbol, yahoo_symbol);

    std::string path = "/quote/" + yahoo_symbol + "/";
    auto response = yahoo_quote_client_->get(path);

    if (!response) {
        return Error{"Yahoo quote page request failed", response.error().message};
    }

    if (response->status_code != 200) {
        return Error{"Yahoo quote page returned error",
                    "HTTP " + std::to_string(response->status_code)};
    }

    const std::string& html = response->body;

    // Pattern 1: HTML label "Earnings Date (est.)" followed by value span
    // e.g. Earnings Date (est.) </span> <span class="value ...">Jul 30, 2026</span>
    std::regex pattern1(R"(Earnings Date[^<]*</span>\s*<span[^>]*>(\w{3}\s+\d{1,2},\s+\d{4}))");
    std::smatch match1;
    if (std::regex_search(html, match1, pattern1) && match1.size() > 1) {
        // Parse "Jul 30, 2026" → "2026-07-30"
        std::string date_str = _parse_month_day_year(match1[1].str());
        if (!date_str.empty()) {
            EarningsDate ed;
            ed.symbol = symbol;
            ed.next_earnings_date = date_str;
            ed.source = "yahoo";
            Logger::debug("Found earnings date (html) for {}: {}", symbol, date_str);
            return ed;
        }
    }

    // Pattern 2: earningsDate with ISO date string (embedded in JS/JSON)
    std::regex pattern2(R"(earningsDate.*?(\d{4}-\d{2}-\d{2}))");
    std::smatch match2;
    if (std::regex_search(html, match2, pattern2) && match2.size() > 1) {
        EarningsDate ed;
        ed.symbol = symbol;
        ed.next_earnings_date = match2[1].str();
        ed.source = "yahoo";
        Logger::debug("Found earnings date (pattern2) for {}: {}", symbol, ed.next_earnings_date);
        return ed;
    }

    // Pattern 3: earningsTimestamp with Unix epoch (in script JSON)
    std::regex pattern3(R"("earningsTimestamp".*?"raw":(\d+))");
    std::smatch match3;
    if (std::regex_search(html, match3, pattern3) && match3.size() > 1) {
        time_t ts = std::stol(match3[1].str());
        std::stringstream ss;
        ss << std::put_time(std::gmtime(&ts), "%Y-%m-%d");
        EarningsDate ed;
        ed.symbol = symbol;
        ed.next_earnings_date = ss.str();
        ed.source = "yahoo";
        Logger::debug("Found earnings date (pattern3) for {}: {}", symbol, ed.next_earnings_date);
        return ed;
    }

    Logger::debug("No earnings date found for {}", symbol);
    return Error{"No earnings date found for " + symbol, "Page may not contain earnings data"};
}

Result<EarningsDate> PriceFetcher::fetch_earnings_date(const std::string& symbol) {
    return fetch_earnings_from_yahoo_quote(symbol);
}

std::map<std::string, EarningsDate> PriceFetcher::fetch_earnings_dates(
    const std::vector<std::string>& symbols) {

    std::map<std::string, EarningsDate> earnings;
    int us_symbols = 0;

    for (const auto& symbol : symbols) {
        // Skip non-US symbols — Yahoo only shows "Earnings Date (est.)" for US stocks
        // HK symbols are numeric (e.g. "700"), JP symbols end in ".T"
        bool is_non_us = false;
        if (symbol.size() > 2 && symbol.substr(symbol.size() - 2) == ".T") is_non_us = true;
        // Check if all digits (HK symbol)
        bool all_digits = !symbol.empty();
        for (char c : symbol) { if (!std::isdigit(c)) { all_digits = false; break; } }
        if (all_digits) is_non_us = true;

        if (is_non_us) {
            Logger::debug("Skipping earnings for non-US symbol: {}", symbol);
            continue;
        }
        ++us_symbols;

        auto result = fetch_earnings_date(symbol);
        if (result) {
            earnings[symbol] = *result;
            Logger::debug("Earnings fetch OK: {} -> {}", symbol, result->next_earnings_date);
        } else {
            Logger::debug("Earnings fetch failed for {}: {}", symbol, result.error().message);
        }

        // Rate limiting: 1.5s between requests to avoid being blocked
        std::this_thread::sleep_for(std::chrono::milliseconds(1500));
    }

    Logger::info("Fetched earnings for {} US symbols, {} successful",
                us_symbols, earnings.size());
    return earnings;
}

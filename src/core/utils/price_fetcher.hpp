...
/**
 * Earnings date information (scraped from Yahoo Finance quote page)
 */
struct EarningsDate {
    std::string symbol;
    std::string next_earnings_date;  // YYYY-MM-DD or empty if not found
...
public:
    ...
    /**
     * Fetch next earnings date for a symbol (scrapes Yahoo Finance quote page).
     */
    Result<EarningsDate> fetch_earnings_date(const std::string& symbol);

    /**
     * Fetch earnings dates for multiple symbols (with rate limiting).
     */
    std::map<std::string, EarningsDate> fetch_earnings_dates(const std::vector<std::string>& symbols);

private:
    std::string alpha_vantage_key_;
    std::unique_ptr<HttpClient> yahoo_client_;
    std::unique_ptr<HttpClient> alpha_vantage_client_;
    std::unique_ptr<HttpClient> yahoo_quote_client_;  // for scraping earnings
    ...
    // Earnings scraping from Yahoo Finance quote page
    Result<EarningsDate> fetch_earnings_from_yahoo_quote(const std::string& symbol);

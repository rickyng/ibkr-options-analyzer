...
Start the interactive dashboard (requires `uv sync --extra dashboard`):
...
Open http://localhost:8001 — four tabs: **Summary** (P&L overview, options/stocks/dividends ledgers), **Positions** (open positions with expiry calendar, exposure, risk charts), **Trade Review** (raw trade history), and **Screener** (put-selling opportunities).

Or via Docker:

```bash
docker compose up -d        # Starts on http://localhost:8001
...
### import-history - Import historical Activity Statement CSVs
```bash
ibkr-options-analyzer import-history [OPTIONS]
```

**Required:**
- `--file TEXT` - CSV file path(s) (1-100 files)

**Optional:**
- `--account TEXT` - Override account name
...
**Required:**
- `TYPE` - Analysis type: `open`, `impact`, `strategy`, `portfolio`, or `screener`
...
- `portfolio` - Consolidated portfolio review with assignment risk alerts, P&L, and expiration calendar
- `screener` - Scan watchlist for put-selling opportunities with grading (A-D)
...
### trades - List option and stock trades
```bash
ibkr-options-analyzer trades [OPTIONS]
```

**Optional:**
- `--date-from TEXT` - Filter trades from this date (YYYY-MM-DD)
- `--date-to TEXT` - Filter trades to this date (YYYY-MM-DD)
- `--underlying TEXT` - Filter by underlying symbol
- `--account TEXT` - Filter by account name
...
| `--google-sheet` | Push output to Google Sheets (configure via config.json) |
...
./build/release/ibkr-options-analyzer trades --date-from 2026-01-01 --date-to 2026-03-31

# Filter by underlying
./build/release/ibkr-options-analyzer trades --underlying AAPL
```

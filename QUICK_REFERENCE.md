...
Open http://localhost:8001 — shows positions, expiry timeline chart + table, exposure, risk distribution, portfolio review, screener, and account filtering.
...
- `--cache-only` - Use cached data only (screener only, no API fetch)
- `--min-iv-percentile FLOAT` - Override screener min IV percentile (0-100)
- `--min-premium-yield FLOAT` - Override screener min annualized yield
- `--min-dte INT` - Override screener min days to expiry
- `--max-dte INT` - Override screener max days to expiry
- `--otm-buffer FLOAT` - Override screener OTM buffer percent
